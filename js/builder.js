...
function addBio(data) {
    ...
    if (personal.resumeUrl != undefined || personal.resumeUrl != '')
        append(domBioLeft, HTMLResumeUrl.replace("%data%",personal.resumeUrl));
    if (personal.contactFormUrl != undefined || personal.contactFormUrl != '')
        append(domBioLeft, HTMLContactFormUrl.replace("%data%",personal.contactFormUrl));

    //right column        
    let domBioRight = document.getElementById("bio-right");
    if (personal.shortBio != undefined && personal.shortBio != '')
        append(domBioRight, HTMLShortBio.replace("%data%", personal.shortBio));
    if (personal.profiles != undefined || data.personal.profiles.length != 0) {
        let social = personal.profiles;
        let socialItems = HTMLSocialStart;
        for (let i in social) {
        ...
        socialItems += HTMLSocialEnd;
        append(domBioRight, socialItems);
    }
}
...
function addEducation(data) {
    ...
    for (var i in education) {
        ...
        var institution = '';
        if (education[i].institutionLink != undefined && education[i].institutionLink != '') {
            institution = HTMLInstitutionName.replace("%data%", education[i].institution);
            institution = institution.replace("%url%", education[i].institutionLink);
        } else {
            institution = HTMLInstitutionNameWithoutUrl.replace("%data%", education[i].institution);
        }
